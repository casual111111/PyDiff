...
```
You can isolate experiment artifacts per writer by setting `writer_name` in your YAML (e.g. `writer_name: alice`) or overriding it via `--writer_name alice`, which saves results to `experiments/<writer_name>` (falls back to `experiments/<exp_name>` when unset) and keeps TensorBoard logs under `tb_logger/<exp_name>/<writer_name>` when provided.
